...
function wedding_setup() {
	 ...
	 * Translations can be filed at WordPress.org. See: https://translate.wordpress.org/projects/wp-themes/wedding
	...
	add_image_size( 'wedding-featured-image', 2000, 1200, true );

	add_image_size( 'wedding-thumbnail-avatar', 100, 100, true );
	 ...
	 * specifically font, colors, and column width.
 	 */
	add_editor_style( array( 'assets/css/editor-style.css', wedding_fonts_url() ) );
	...
	$starter_content = array(
		...
		'attachments' => array(
			'image-espresso' => array(
				'post_title' => _x( 'Espresso', 'Theme starter content', 'wedding' ),
				'file' => 'assets/images/espresso.jpg', // URL relative to the template directory.
			),
			'image-sandwich' => array(
				'post_title' => _x( 'Sandwich', 'Theme starter content', 'wedding' ),
				'file' => 'assets/images/sandwich.jpg',
			),
			'image-coffee' => array(
				'post_title' => _x( 'Coffee', 'Theme starter content', 'wedding' ),
		...
		'nav_menus' => array(
			...
			'social' => array(
				'name' => __( 'Social Links Menu', 'wedding' ),
				'items' => array(
					'link_yelp',
					'link_facebook',
					'link_twitter',
	 ...
	 */
	$starter_content = apply_filters( 'wedding_starter_content', $starter_content );
...
function wedding_content_width() {
	 ...
	 */
	$GLOBALS['content_width'] = apply_filters( 'wedding_content_width', $content_width );
}
add_action( 'template_redirect', 'wedding_content_width', 0 );
...
function wedding_fonts_url() {
	 ...
	 */
	$libre_franklin = _x( 'on', 'Libre Franklin font: on or off', 'wedding' );
...
function wedding_resource_hints( $urls, $relation_type ) {
	if ( wp_style_is( 'wedding-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
		$urls[] = array(
...
/*function wedding_widgets_init() {
	register_sidebar( array(
		'name'          => __( 'Blog Sidebar', 'wedding' ),
		'id'            => 'sidebar-1',
		'description'   => __( 'Add widgets here to appear in your sidebar on blog posts and archive pages.', 'wedding' ),
 ...
 */
function wedding_excerpt_more( $link ) {
	if ( is_admin() ) {
		return $link;
	}
	...
	$link = sprintf( '<p class="link-more"><a href="%1$s" class="more-link">%2$s</a></p>',
					...
					sprintf( __( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'wedding' ), get_the_title( get_the_ID() ) )
				   );
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'wedding_excerpt_more' );
 ...
 * @since Twenty Seventeen 1.0
 */
function wedding_javascript_detection() {
	echo "<script>(function(html){html.className = html.className.replace(/\bno-js\b/,'js')})(document.documentElement);</script>\n";
}
add_action( 'wp_head', 'wedding_javascript_detection', 0 );
...
function wedding_style_script()
{
	...
	//Scripts
	wp_enqueue_script( 'bootstrap', get_theme_file_uri( '/assets/lib/bootstrap/dist/js/bootstrap.min.js' ), array( 'jquery' ), '2.1.2', true );
	wp_enqueue_script( 'wow', get_theme_file_uri( '/assets/lib/wow/dist/wow.js' ), array( 'jquery' ), '2.1.2', true );
	wp_enqueue_script( 'YTPlayer', get_theme_file_uri( '/assets/lib/jquery.mb.ytplayer/dist/jquery.mb.YTPlayer.js' ), array( 'jquery' ), '2.1.2', true );
	wp_enqueue_script( 'isotope', get_theme_file_uri( '/assets/lib/isotope/dist/isotope.pkgd.js' ), array( 'jquery' ), '2.1.2', true );
	wp_enqueue_script( 'imagesloaded', get_theme_file_uri( '/assets/lib/imagesloaded/imagesloaded.pkgd.js' ), array( 'jquery' ), '2.1.2', true );
 ...
 */
function wedding_content_image_sizes_attr( $sizes, $size ) {
	$width = $size[0];
 ...
 */
function wedding_header_image_tag( $html, $header, $attr ) {
	if ( isset( $attr['sizes'] ) ) {
		$html = str_replace( $attr['sizes'], '100vw', $html );
	}
	return $html;
}
add_filter( 'get_header_image_tag', 'wedding_header_image_tag', 10, 3 );
 ...
 */
function wedding_post_thumbnail_sizes_attr( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'wedding_post_thumbnail_sizes_attr', 10, 3 );
 ...
 */
function wedding_front_page_template( $template ) {
	return is_home() ? '' : $template;
}
add_filter( 'frontpage_template',  'wedding_front_page_template' );
 ...
 */
function wedding_widget_tag_cloud_args( $args ) {
	$args['largest']  = 1;
	$args['smallest'] = 1;
	$args['unit']     = 'em';
	$args['format']   = 'list';
